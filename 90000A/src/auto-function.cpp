...
void quickTurnTo(double _tarAng, double _maxSpeed)
{
    ...
    if ( fabs(_tarAng - IMUHeading()) < 10 || fabs(fabs(_tarAng - IMUHeading()) -360) < 10 ) {
        pidr.setCoefficient(10, 0.0, 0.05); // for small angle
    } 
...
void quickMoveDistance(double _distance, double _maxSpeed, double _maxTime, double kdiff)
{
    ...
    {
        double leftPosition = LeftMotor.position(rev);
        double rightPosition = RightMotor.position(rev);
        double revolution = (leftPosition + rightPosition)/2 ;
        double accumulatedLength = M_PI * Diameter * revolution * GearRatio;
        double d = _distance - accumulatedLength;
        double dSide = leftPosition - rightPosition;
        ...
        double dPower = dSide * kdiff;
        LeftMotor.spin(vex::directionType::fwd, velocityV*127 - dPower, voltageUnits::mV);
        RightMotor.spin(vex::directionType::fwd, velocityV*127 + dPower, voltageUnits::mV);   
        this_thread::sleep_for(20);    
    }
    LeftMotor.stop(hold);
    RightMotor.stop(hold);
